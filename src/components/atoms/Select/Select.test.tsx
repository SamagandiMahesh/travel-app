...
describe('ODSelect', () => {
  it('renders without crashing', () => {
    const { getByTestId } = render(<ODSelect options={[]} onChange={jest.fn()} />);
  ...
  it('calls onChange when the selected option changes', async () => {
    ...
    const { container } = render(<ODSelect options={options} onChange={mockOnChange} />);

    // Simulate opening the dropdown
    const dropdownIndicator = container.querySelector('.od__dropdown-indicator');
  ...
  it('displays the placeholder when no option is selected', () => {
    const { container } = render(<ODSelect options={[]} onChange={jest.fn()} />);
